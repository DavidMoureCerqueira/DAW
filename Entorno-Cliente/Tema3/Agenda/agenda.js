...
function pintarEventos(eventosF) {
    eventosF.sort((a, b) => a.id - b.id)
    posicionEventos.innerText = '';
    for (let evento of eventosF) {
        let eventoLi = document.createElement('li')
        eventoLi.classList.add('evento')
        eventoLi.setAttribute('id',evento.id)
        let eventoPrimerDiv = document.createElement('div')
        eventoPrimerDiv.textContent = (evento.id).toLocaleDateString('gl-ES', options);
        eventoLi.appendChild(eventoPrimerDiv)
        let eventoSegundoDiv = document.createElement('div')
        eventoSegundoDiv.textContent = `${evento.nombre}`
        eventoLi.appendChild(eventoSegundoDiv)
        let botonModificar = document.createElement('button')
        botonModificar.textContent = '✏️'
        botonModificar.addEventListener('click', () => { 
            eventoLi.remove()
            fechaEventoInput.valueAsDate=evento.id
            nombreEventoInput.value=evento.nombre
            eventos=eventosF.filter((eventoFun)=>eventoFun.id!=evento.id)
        })

        eventoLi.appendChild(botonModificar)
        ...
        eventoLi.appendChild(botonEliminar)

        posicionEventos.appendChild(eventoLi)

        // posicionEventos.insertAdjacentHTML('beforeend', `<li class='evento'> <div>${(evento.id).toLocaleDateString('gl-ES', options)}</div> 
        //  <div>${evento.nombre}</div> 
        // <button type='button' id='${evento.id}' onClick="modificarEvento(id)">✏️</button>
        // <button type='button' id='${evento.id}' onClick="eliminarEvento(id)">✖️</button></li>`)
    }
